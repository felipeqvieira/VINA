...
void inserir_membros(Archiver *archiver, const char **nomes_arquivos, int num_arquivos) {
    for (int i = 0; i < num_arquivos; i++) {
        const char *nome_membro = nomes_arquivos[i];
        ...
        while ((bytes_lidos = fread(buffer, sizeof(unsigned char), MAX_CONTEUDO, arquivo_membro)) > 0) {
            // Armazena o conteúdo lido em um novo arquivo dentro do archiver
            FILE *arquivo_archiver = fopen(nome_membro, "ab");
            if (arquivo_archiver == NULL) {
                printf("Erro ao criar o arquivo do membro '%s' dentro do ArchiveData.\n", nome_membro);
                fclose(arquivo_membro);
                break;  // Interrompe a leitura do arquivo atual e passa para o próximo arquivo
            }
        ...
        verificar_conteudo_membro(nome_membro, &(archiver->archiveData), novo_membro.tamanho);
